...
void populate_state_table() {
  ...
  // Initializing all handler functions to default display 
...
void display_altitude_state(void) {
  LCD.clear();
  if (rel_or_abs_hgt) {
    print_altitude(get_altitude(press_mov_avg, temp));
  } else {
    print_altitude(get_altitude(press_mov_avg, temp) - curr_alt_offset);
  }
  print_temparature(temp);
}
...
void setup () {
  ...
  MsTimer2::set(500, timer_handler);
  MsTimer2::start();
...
void loop() {
  ...
  print_data_to_serial(pressure, temp);
...
void print_altitude(float altitude) {
  LCD.setCursor(0,0);
  LCD.print("H:");
...
void print_data_to_serial(float pressure, float temp) {
